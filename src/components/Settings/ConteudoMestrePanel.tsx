...
export function ConteudoMestrePanel({ campanhaId, initialAba = 'lore', initialItem, onDirtyChange }: ConteudoMestrePanelProps) {
  ...
  const exportar = async () => {
    ...
    try {
      const snapshot = await conteudoEditorialApi.exportarPublicados(campanhaId);
      ...
      setExportMessage('Snapshot baixado. Antes do commit, sincronize esse arquivo com o repositório pelo comando editorial:sync.');
    } catch (error: any) {
  ...
  return (
    <div>
      <div className="mb-5 flex flex-col gap-3 border-b border-white/10 sm:flex-row sm:items-end sm:justify-between">
        <div className="horizontal-scroll flex gap-3 overflow-x-auto">
          <button type="button" onClick={() => trocarAba('lore')} className={`flex items-center gap-2 border-b-2 px-2 pb-3 text-xs font-bold uppercase tracking-widest ${aba === 'lore' ? 'border-primary text-primary' : 'border-transparent text-gray-500'}`}><BookOpenText size={15} /> Lore</button>
          <button type="button" onClick={() => trocarAba('cronologia')} className={`flex items-center gap-2 border-b-2 px-2 pb-3 text-xs font-bold uppercase tracking-widest ${aba === 'cronologia' ? 'border-primary text-primary' : 'border-transparent text-gray-500'}`}><Clock3 size={15} /> Cronologia</button>
          <button type="button" onClick={() => trocarAba('loja')} className={`flex items-center gap-2 border-b-2 px-2 pb-3 text-xs font-bold uppercase tracking-widest ${aba === 'loja' ? 'border-primary text-primary' : 'border-transparent text-gray-500'}`}><Store size={15} /> Loja</button>
          <button type="button" onClick={() => trocarAba('regras')} className={`flex items-center gap-2 border-b-2 px-2 pb-3 text-xs font-bold uppercase tracking-widest ${aba === 'regras' ? 'border-primary text-primary' : 'border-transparent text-gray-500'}`}><Library size={15} /> Regras</button>
        </div>
      ...
      {aba === 'lore' && <ConteudoLorePanel campanhaId={campanhaId} onDirtyChange={registrarDirty} />}
      {aba === 'cronologia' && <CronologiaPanel campanhaId={campanhaId} onDirtyChange={registrarDirty} />}
