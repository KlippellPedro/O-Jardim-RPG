...
import { Check, Clock3, History, Loader2, LockKeyhole, Plus, RotateCcw, Save, Search, Send, Trash2 } from 'lucide-react';
import {
...
export function RegrasEditorPanel({ campanhaId, initialItem, onDirtyChange }: RegrasEditorPanelProps) {
  ...
  const destaques = Array.isArray(conteudo.destaques)
    ? conteudo.destaques.filter((item): item is string[] => Array.isArray(item)).map((item) => [String(item[0] || ''), String(item[1] || '')])
    : [];
  if (loading) {
            ...
            <div className="mx-auto max-w-4xl space-y-6">
              ...
              {selecionada.tipo === 'regra' ? (
                <div className="space-y-5">
                  ...
                  <div>
                    <div className="mb-2 flex items-center justify-between"><label className="text-[10px] font-bold uppercase tracking-widest text-gray-500">Destaques</label><button type="button" onClick={() => atualizarCampo('destaques', [...destaques, ['', '']])} className="inline-flex items-center gap-1 text-xs text-primary"><Plus size={13} /> Adicionar</button></div>
                    <div className="space-y-2">{destaques.map((item, index) => <div key={`destaque-${index}`} className="grid gap-2 sm:grid-cols-[minmax(0,0.7fr)_minmax(0,1.3fr)_auto]"><input aria-label={`Título do destaque ${index + 1}`} value={item[0]} onChange={(event) => atualizarCampo('destaques', destaques.map((atual, idx) => idx === index ? [event.target.value, atual[1]] : atual))} placeholder="Rótulo" maxLength={240} className="rounded-xl border border-white/10 bg-black/40 px-3 py-2 text-sm text-white outline-none focus:border-primary/50" /><input aria-label={`Texto do destaque ${index + 1}`} value={item[1]} onChange={(event) => atualizarCampo('destaques', destaques.map((atual, idx) => idx === index ? [atual[0], event.target.value] : atual))} placeholder="Explicação" maxLength={240} className="rounded-xl border border-white/10 bg-black/40 px-3 py-2 text-sm text-white outline-none focus:border-primary/50" /><button type="button" aria-label={`Remover destaque ${index + 1}`} onClick={() => atualizarCampo('destaques', destaques.filter((_, idx) => idx !== index))} className="rounded-xl border border-white/10 p-2.5 text-gray-500 hover:text-red-300"><Trash2 size={15} /></button></div>)}</div>
                  </div>
