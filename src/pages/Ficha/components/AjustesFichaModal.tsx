  ...
  return (
    <FichaModal isOpen={isOpen} onClose={onClose} title={`AJUSTES: ${titulo.toUpperCase()}`}>
      <div className="space-y-5">
        ...
        <div className="grid gap-3 rounded-xl border border-dashed border-[#c7a44c]/25 bg-[#c7a44c]/5 p-4 sm:grid-cols-[1fr_7rem_auto]">
          <input
            value={nome}
            onChange={(event) => setNome(event.target.value)}
            placeholder="Fonte: item, bênção, ferimento..."
            className="rounded-md border border-white/10 bg-black/30 px-3 py-2 text-sm text-white outline-none focus:border-[#c7a44c]/50"
          />
          <input
            value={valor}
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 rounded-md bg-[#c7a44c] px-4 py-2 text-sm font-bold text-black disabled:opacity-40"
          >
