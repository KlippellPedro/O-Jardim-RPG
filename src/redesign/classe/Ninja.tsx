import { motion } from 'framer-motion';
import { Star, CircleArrowUp, Swords, Ghost, Gauge } from 'lucide-react';
import type { IClasse } from '../../types/catalogo';
import { PremiumCard } from '../components/premium/PremiumCard';
import { DetalhesClasse } from '../../pages/Regras/components/DetalhesClasse';
import { obterTemaPorId } from '../themeMap';

export const Ninja = ({ classe }: { classe: IClasse }) => {
  const tema = obterTemaPorId(classe.id);

  return (
    <div className="min-h-screen text-gray-50 p-8 selection:bg-gray-500/30 overflow-hidden relative">
      {/* 3D WebGL Background */}
      <div className="fixed inset-0 z-0 bg-cover bg-center bg-no-repeat opacity-20 pointer-events-none" style={{ backgroundImage: "url('/assets/img/ninja_class_fantasy_bg.webp')" }} />

      {/* Stealth / Ninja Background Effects */}
      <div className="fixed inset-0 pointer-events-none z-0">
        <div className="absolute top-[30%] left-[30%] w-[40%] h-[40%] bg-zinc-900/10 blur-[150px] rounded-full mix-blend-screen" />

        {/* Shurikens flying randomly */}
        {[...Array(5)].map((_, i) => (
          <motion.div
            key={i}
            className="absolute text-zinc-800"
            style={{
              left: `${Math.random() * 100}%`,
              top: `${Math.random() * 100}%`,
            }}
            animate={{
              x: [0, Math.random() * 400 - 200],
              y: [0, Math.random() * 400 - 200],
              rotate: 1080,
              opacity: [0, 0.4, 0],
            }}
            transition={{
              duration: 1.5,
              repeat: Infinity,
              ease: "linear",
              delay: Math.random() * 4
            }}
          >
            <Star size={40} />
          </motion.div>
        ))}
      </div>

      <div className="max-w-5xl mx-auto relative z-10 pt-16">
        {/* Header */}
        <motion.header
          initial={{ opacity: 0, y: -20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6, ease: "easeOut" }}
          className="text-center mb-24"
        >
          <motion.div
            className={`w-28 h-28 mx-auto ${tema.bg} ${tema.border} rounded-full flex items-center justify-center mb-8 relative backdrop-blur-md`}
            style={{ boxShadow: `0 0 50px ${tema.glow}` }}
          >
            <Star size={48} className={tema.icon} strokeWidth={1} />
          </motion.div>

          <motion.div className="overflow-hidden">
             <motion.h1
                initial={{ y: "100%" }}
                animate={{ y: 0 }}
                transition={{ duration: 0.8, delay: 0.2, ease: [0.16, 1, 0.3, 1] }}
                className={`text-7xl font-bold tracking-tight ${tema.text} mb-6 uppercase`}
                style={{ fontFamily: 'Cinzel, serif' }}
             >
               Ninja
             </motion.h1>
          </motion.div>
          <motion.p
             initial={{ opacity: 0 }}
             animate={{ opacity: 1 }}
             transition={{ duration: 1, delay: 0.6 }}
             className="text-lg text-zinc-400 max-w-2xl mx-auto font-medium leading-relaxed"
          >
            Silenciosos, letais e regidos por uma hierarquia estrita. A verdadeira extensão de seus corpos é a sua Arma Ninja, aprimorada até o limite da letalidade física.
          </motion.p>
        </motion.header>

        {/* Traits Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-24">
          <PremiumCard
            glowColor={tema.glow}
            initial={{ opacity: 0, x: -30 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.5 }}
            className={`flex flex-col p-8 ${tema.bg} ${tema.border} backdrop-blur-md`}
          >
            <CircleArrowUp size={36} className={`${tema.icon} mb-6`} strokeWidth={1.5} />
            <h3 className={`text-2xl font-bold ${tema.text} mb-3 uppercase tracking-wide`} style={{ fontFamily: 'Cinzel, serif' }}>Hierarquia</h3>
            <p className="text-zinc-400 leading-relaxed text-sm">
              Uma escada de posto dentro do clã, do Aprendiz ao Kage. Cada degrau soma Furtividade ao anterior, de +1 a +5, e libera algo que o clã oferece: acesso aos arquivos, apoio logístico, comando sobre Gennin, peso junto aos anciãos. No topo, o Kage é o líder da vila: autoridade final sobre missões, alianças e disputas territoriais do seu povo.
            </p>
          </PremiumCard>

          <PremiumCard
            glowColor={tema.glow}
            initial={{ opacity: 0, x: 30 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.5, delay: 0.2 }}
            className={`flex flex-col p-8 ${tema.bg} ${tema.border} backdrop-blur-md`}
          >
            <Swords size={36} className={`${tema.icon} mb-6`} strokeWidth={1.5} />
            <h3 className={`text-2xl font-bold ${tema.text} mb-3 uppercase tracking-wide`} style={{ fontFamily: 'Cinzel, serif' }}>Arma Ninja</h3>
            <p className="text-zinc-400 leading-relaxed text-sm">
              Marque uma arma como sua a cada estágio e aplique uma melhoria da lista, como Fio Mortal, Golpe Certeiro ou Retorno da Sombra, a qualquer arma já marcada. Na mesma arma, só melhorias com bônus numérico podem se repetir; efeitos sem número ou que não acumulam valem uma vez.
            </p>
          </PremiumCard>

          <PremiumCard
            glowColor={tema.glow}
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.5, delay: 0.4 }}
            className={`flex flex-col p-8 ${tema.bg} ${tema.border} backdrop-blur-md`}
          >
            <Ghost size={36} className={`${tema.icon} mb-6`} strokeWidth={1.5} />
            <h3 className={`text-2xl font-bold ${tema.text} mb-3 uppercase tracking-wide`} style={{ fontFamily: 'Cinzel, serif' }}>Sombra Suprema</h3>
            <p className="text-zinc-400 leading-relaxed text-sm">
              Uma vez por sessão, ative sem gastar ação: por três rodadas seu deslocamento não provoca reações, o primeiro ataque de cada turno vem com vantagem e +2 dados de dano, e você fica impossível de mirar diretamente até agir. Guarde uma Reação para transformar um acerto contra você em erro.
            </p>
          </PremiumCard>
          <PremiumCard
            glowColor={tema.glow}
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 0.5, delay: 0.6 }}
            className={`flex flex-col p-8 ${tema.bg} ${tema.border} backdrop-blur-md`}
          >
            <Gauge size={36} className={`${tema.icon} mb-6`} strokeWidth={1.5} />
            <h3 className={`text-2xl font-bold ${tema.text} mb-3 uppercase tracking-wide`} style={{ fontFamily: 'Cinzel, serif' }}>Sobre a DT</h3>
            <p className="text-zinc-400 leading-relaxed text-sm">
              Toda técnica sua que obriga alguém a resistir rola Furtividade no momento em que aciona: o resultado vira a DT que o alvo precisa alcançar, valendo para todos os atingidos naquele uso.
            </p>
          </PremiumCard>
        </div>

        <DetalhesClasse classe={classe} />
      </div>
    </div>
  );
};

export default Ninja;
