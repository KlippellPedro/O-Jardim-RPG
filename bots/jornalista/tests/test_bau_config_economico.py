"""/bau_config ganhou chance_enigma_percent/lunaris_min/lunaris_max: chance
de baú trancado e faixa de Lunaris eram constantes fixas pro Jardim
inteiro (core/loot.py) e passam a poder ser ajustadas por servidor."""
...
def test_bau_config_repassa_chance_e_faixa_de_lunaris():
    db = _DB()
    cog = _cog(db)
    ...
    _rodar(
        Baus.bau_config.callback(
            ...
            chance_enigma_percent=40, lunaris_min=10, lunaris_max=20,
        )
    )

    guild_id, kwargs = db.chamadas_atualizar[0]
    assert kwargs["chance_enigma_percent"] == 40
...
def test_bau_config_recusa_lunaris_min_maior_que_max():
    ...
    _rodar(
        Baus.bau_config.callback(
            cog, interacao,
            canal=None, ativo=None, min_hora=None, max_hora=None, itens_por_bau=None,
            chance_enigma_percent=None, lunaris_min=50, lunaris_max=10,
        )
    )

    assert not db.chamadas_atualizar
    texto, ephemeral = interacao.response.mensagens[0]
    assert "lunaris_min" in texto


def test_bau_config_resposta_mostra_faixa_e_chance_resolvidas():
    db = _DB()
    cog = _cog(db)
    interacao = _Interacao(100)
    ...
    _rodar(
        Baus.bau_config.callback(
            cog, interacao,
            canal=None, ativo=None, min_hora=None, max_hora=None, itens_por_bau=None,
            chance_enigma_percent=None, lunaris_min=None, lunaris_max=None,
        )
    )
