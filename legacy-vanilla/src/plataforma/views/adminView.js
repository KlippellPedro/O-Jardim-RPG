...
function renderManutencao(area, ctx) {
  ...
  const estadoAutomatico = elemento('p', 'plataforma-backup-estado', 'Consultando a rotina automática…');
  automacao.append(
    elemento('h4', '', 'Backup automático'),
    estadoAutomatico,
    elemento('small', '', 'A cópia automática fica no armazenamento do app e ajuda em exclusões acidentais. '
      + 'Continue baixando uma cópia externa de tempos em tempos: ela é a proteção contra perda total do servidor.'),
  );
  card.append(automacao);
  area.append(card);

  adminApi.backupAutomatico().then((resultado) => {
    if (!resultado.ativo) {
      estadoAutomatico.textContent = 'Desligado neste ambiente.';
      return;
    }
    const rotina = `Ativo · a cada ${resultado.intervalo_horas}h · mantém ${resultado.retencao} cópias`;
    if (resultado.ultimo_erro) {
      estadoAutomatico.textContent = `${rotina}. A última tentativa falhou: ${resultado.ultimo_erro}`;
      estadoAutomatico.dataset.estado = 'erro';
      return;
    }
    estadoAutomatico.textContent = resultado.ultimo_em
      ? `${rotina}. Última cópia ${dataRelativa(resultado.ultimo_em)} (${resultado.ultimo_arquivo}).`
      : `${rotina}. A primeira cópia será criada pelo servidor.`;
  }).catch((erro) => {
    estadoAutomatico.textContent = `Não foi possível consultar a rotina: ${texto(erro)}`;
    estadoAutomatico.dataset.estado = 'erro';
  });
}
...
async function renderUsuarios(area, ctx) {
  ...
  const buscaCampo = campo('Buscar contas', 'search', 'busca', {
    placeholder: 'Nome ou e-mail',
    autocomplete: 'off',
  });
  const busca = buscaCampo.input;
  const filtroPapel = seletor('Cargo', [
    ['', 'Todos os cargos'],
    ['player', 'Player'],
    ['mestre', 'Mestre'],
    ['admin', 'Administrador'],
  ...
  const filtroAtivo = seletor('Situação', [
    ['', 'Ativos e desativados'],
    ['true', 'Somente ativos'],
    ['false', 'Somente desativados'],
  ], '');
  filtros.append(buscaCampo, filtroPapel, filtroAtivo);
  ...
  async function carregar() {
    ...
    try {
      ...
      usuarios.forEach(usuario => {
        ...
        const papel = seletor('Cargo', protegido
          ? [['criador', 'Criador']]
