...
import { navegarComCarregamento } from '../../core/navigationLoader.js?v=1';
import {
  ROTULOS_ACESSO,
  ROTULOS_MODULO,
  ROTULOS_PAPEL,
...
async function renderMesa(area, ctx) {
  ...
  membros.forEach(membro => {
    ...
    const papel = seletor('Papel na mesa', Object.entries(ROTULOS_PAPEL), membro.papel);
  ...
  form.append(acao('Gerar convite', '', async () => {
    try {
      ...
      const codigo = elemento('strong', '', resposta.codigo);
      const copiar = botao('Copiar código', 'plataforma-botao--secundario plataforma-botao--mini', async () => {
        try {
          await navigator.clipboard.writeText(resposta.codigo);
          copiar.textContent = 'Código copiado';
        } catch {
          copiar.textContent = 'Selecione o código';
        }
      });
      const linhaCodigo = elemento('div', 'plataforma-codigo-linha');
...
async function renderPersonagens(area, ctx) {
  ...
  const buscaCampo = campo('Buscar personagens', 'search', 'busca', {
    placeholder: 'Nome do personagem ou jogador',
    autocomplete: 'off',
  });
  const busca = buscaCampo.input;
  const porDono = seletor('Jogador', [
    ['todos', 'Todos os jogadores'],
    ...(estado.detalhes?.membros || []).map(membro => [membro.id, membro.nome_exibicao]),
  ], 'todos');
  filtros.append(buscaCampo, porDono);
  ...
  function desenhar() {
    ...
    itens.forEach(personagem => {
      ...
      acoes.append(botao('Abrir ficha', 'plataforma-botao--secundario', () => {
        navegarComCarregamento(`/ficha#/personagem/${personagem.id}`);
      }));
...
async function renderConteudo(area, ctx) {
  ...
  const buscaCampo = campo('Buscar conteúdo', 'search', 'busca', {
    placeholder: 'Título na biblioteca',
    autocomplete: 'off',
  });
  const busca = buscaCampo.input;
  ...
  controles.append(modulo, buscaCampo, filtroStatus);
...
async function renderLiberacoes(area, ctx) {
  ...
  const controles = elemento('div', 'plataforma-controles');
  const buscaCampo = campo('Buscar informação', 'search', 'busca', {
    placeholder: 'Título publicado',
    autocomplete: 'off',
  });
  const busca = buscaCampo.input;
  ...
  controles.append(buscaCampo, filtroVisibilidade);
