...
import { renderTrocaObrigatoria } from './views/senhaView.js?v=2';
import { ROTULOS_PAPEL, areaTexto, botao, campo, carregando, elemento, texto } from './views/ui.js';

const CHAVE_CAMPANHA = 'jardim-campanha-ativa';
const CHAVE_CONTEXTO_CACHE = 'jardim-contexto-recente-v1';
const CONTEXTO_CACHE_MS = 2 * 60 * 1000;
...
let versaoRenderPainel = 0;

const carregarView = {
  conta: () => import('./views/contaView.js?v=4').then(modulo => modulo.renderConta),
  avisos: () => import('./views/avisosView.js?v=2').then(modulo => modulo.renderAvisos),
  cofre: () => import('./views/contaView.js?v=4').then(modulo => modulo.renderCofre),
  campanhas: () => import('./views/campanhasView.js?v=2').then(modulo => modulo.renderCampanhas),
  mestre: () => import('./views/mestreView.js?v=3').then(modulo => modulo.renderMestre),
  admin: () => import('./views/adminView.js?v=2').then(modulo => modulo.renderAdmin),
};
...
function limparEstado() {
  ...
  try {
    sessionStorage.removeItem(CHAVE_CONTEXTO_CACHE);
  } catch {
    // Armazenamento pode estar bloqueado; a plataforma continua pela API.
  }
}

function aplicarContexto(contexto) {
  estado.usuario = contexto.usuario || null;
  ...
  estado.avisosNaoLidos = contexto.avisos_nao_lidos ?? contexto.avisosNaoLidos ?? 0;
  if (estado.campanha) localStorage.setItem(CHAVE_CAMPANHA, estado.campanha.id);
  else localStorage.removeItem(CHAVE_CAMPANHA);
  atualizarDock();
}

function salvarContextoRecente() {
  try {
    sessionStorage.setItem(CHAVE_CONTEXTO_CACHE, JSON.stringify({
      salvoEm: Date.now(),
      contexto: obterContextoPlataforma(),
    }));
  } catch {
    // Cache é só aceleração; falhar aqui não pode impedir a navegação.
  }
}

function restaurarContextoRecente() {
  try {
    const salvo = JSON.parse(sessionStorage.getItem(CHAVE_CONTEXTO_CACHE) || 'null');
    if (!salvo?.contexto || Date.now() - Number(salvo.salvoEm) > CONTEXTO_CACHE_MS) return null;
    const campanhaPreferida = localStorage.getItem(CHAVE_CAMPANHA);
    if (campanhaPreferida && salvo.contexto.campanha?.id !== campanhaPreferida) return null;
    return salvo.contexto;
  } catch {
    return null;
  }
}
...
function atualizarCabecalho(titulo, contexto = 'Acesso à plataforma') {
  if (!raiz) return;
  raiz.querySelector('.plataforma-titulo').textContent = titulo;
  raiz.querySelector('.plataforma-topo-contexto').textContent = contexto;
}
...
function renderAjudaSenha() {
  atualizarCabecalho('Recuperar acesso', 'Pedido de senha provisória');
  ...
  form.append(campo('Seu e-mail', 'email', 'email', {
    required: true,
    autocomplete: 'email',
...
function renderAuth() {
  atualizarCabecalho('Acessar o Jardim', modoAuth === 'cadastro' ? 'Criar uma conta' : 'Conta da plataforma');
  painel.innerHTML = `
  ...
  if (modoAuth === 'cadastro') {
    ...
    if (exigeConvite) {
      ...
      form.append(campo('Código do convite', 'text', 'convite', {
        ...
        classe: 'plataforma-campo--codigo',
      }));
    }
    form.append(campo('Nome de exibição', 'text', 'nome', {
      required: true,
      minlength: 2,
      maxlength: 80,
      autocomplete: 'name',
    ...
    form.append(campo('E-mail', 'email', 'email', {
      required: true,
      autocomplete: 'email',
      placeholder: 'voce@exemplo.com',
    }));
    form.append(campo('Senha', 'password', 'senha', {
      required: true,
      minlength: 8,
      maxlength: 128,
      autocomplete: 'new-password',
      descricao: 'Use pelo menos 8 caracteres.',
    }));
...
function renderEntradaCampanha() {
  ...
  atualizarCabecalho(
    modoCampanha === 'criar' ? 'Criar campanha' : 'Escolher campanha',
    estado.usuario?.nome_exibicao || 'Conta conectada',
  );
  ...
  if (modoCampanha === 'criar') {
    ...
    form.append(campo('Nome da campanha', 'text', 'nome', {
      required: true,
      minlength: 2,
      maxlength: 100,
      placeholder: 'Ex.: Ecos de Alétheia',
    }));
    form.append(areaTexto('Descrição', 'descricao', {
      maxlength: 2000,
      rows: 4,
      placeholder: 'Um resumo curto para orientar quem entrar na mesa.',
    }));
  ...
  form.append(campo('Código do convite', 'text', 'codigo', {
    required: true,
    minlength: 6,
    maxlength: 32,
    autocomplete: 'off',
...
function criarNavegacao(abaAtiva) {
  ...
  nav.setAttribute('aria-label', 'Áreas da plataforma');
  ...
  abas.forEach(([id, rotulo]) => {
    ...
    if (id === abaAtiva) item.setAttribute('aria-current', 'page');
...
function seletorCampanha() {
  ...
  estado.campanhas.forEach(campanha => {
    ...
    option.textContent = `${campanha.nome} · ${ROTULOS_PAPEL[campanha.papel] || campanha.papel}`;
...
const ctx = {
  ...
  async sair() {
    const { encerrarSessao } = await import('./views/contaView.js?v=4');
...
async function renderPainel(aba = 'conta', secao = null) {
  const minhaVersao = ++versaoRenderPainel;
  const titulos = {
    conta: 'Minha conta',
    avisos: 'Avisos',
    cofre: 'Cofre de recompensas',
    campanhas: 'Campanhas',
    mestre: 'Painel do mestre',
    admin: 'Administração',
  };
  atualizarCabecalho(
    titulos[aba] || 'Conta e campanha',
    estado.campanha?.nome || estado.usuario?.nome_exibicao || 'Acesso à plataforma',
  );
  painel.replaceChildren();
  ...
  painel.append(area);
  area.append(carregando('Abrindo área…'));

  try {
    const desenhar = await (carregarView[aba] || carregarView.conta)();
    if (minhaVersao !== versaoRenderPainel) return;
    area.replaceChildren();
    await desenhar(area, ctx, secao);
  } catch (erro) {
    if (minhaVersao === versaoRenderPainel) {
      area.replaceChildren();
      informar(texto(erro), 'erro');
    }
  }
...
function renderInicial() {
  ...
  if (precisaTrocarSenha()) {
    atualizarCabecalho('Definir nova senha', estado.usuario.nome_exibicao);
    return renderTrocaObrigatoria(painel, ctx);
  }
...
export async function inicializarPlataforma({ exigirCampanha = true } = {}) {
  ...
  if (!inicializacao) {
    const contextoRecente = restaurarContextoRecente();
    if (contextoRecente) {
      aplicarContexto(contextoRecente);
    } else {
  ...
  // Em uma troca recente de página, o contexto validado anterior já permite
  // desenhar o módulo. A API revalida em segundo plano e publica qualquer
  // mudança; no primeiro acesso continuamos esperando a resposta real.
  const cachePodeAbrir = estado.usuario
    && !precisaTrocarSenha()
    && (!exigirCampanha || estado.campanha || podeContinuarSemCampanha());
  if (!cachePodeAbrir) await inicializacao;
  atualizarDock();
