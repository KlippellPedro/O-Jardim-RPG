...
import { inicializarPlataforma } from '../plataforma/portal.js?v=8';
...
async function atualizarCarteiraExibida() {
  ...
  try {
    ...
    // Falha ao atualizar o saldo em tela não deve mascarar a compra já feita.
  }
}

function criarBotaoAcao(categoria, entrada, extraClasse) {
  const btn = document.createElement('button');
  btn.type = 'button';
  ...
  btn.addEventListener('click', async (e) => {
    ...
    if (!personagemAtivoId) {
      mostrarToast('Escolha um personagem ativo (na Conta) antes de comprar.', 'info');
      return;
    }
    const moeda = getMoedaAtiva();
    const valor = precoParaMoeda(entrada, moeda);
    if (typeof valor !== 'number') {
      mostrarToast(`${entrada.titulo} não tem preço em ${moeda.nome}.`, 'info');
      return;
    }
    btn.disabled = true;
    ...
    try {
      const { cofreApi } = await import('../plataforma/cofreApi.js');
      const compra = await cofreApi.comprar(campanhaAtivaId, entrada.id, moeda.nome);
      mostrarToast(
        `${compra.personagem?.nome || 'Você'} comprou ${compra.item} por ${compra.preco} ${compra.moeda}. Restam ${compra.restante}.`,
        'sucesso',
      );
      await atualizarCarteiraExibida();
    } catch (erro) {
      mostrarToast(erro.message || 'Não foi possível comprar.', 'erro');
    } finally {
      btn.disabled = false;
...
function criarItemCard(entrada, indice) {
  ...
  rodape.appendChild(criarBotaoAcao(categoria, entrada));
...
function abrirModalItem(catId, itemId) {
  ...
  rodape.appendChild(criarBotaoAcao(categoria, entrada, 'loja-modal-acao'));
  corpo.appendChild(rodape);
...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    personagemAtivoId = contexto.campanha.personagem_ativo_id || null;
    if (personagemAtivoId) {
      const detalhe = await personagensApi.obter(personagemAtivoId);
      definirMoedasRemotas(detalhe.personagem?.carteira || []);
    }
