...
async function comCatalogo(renderizar) {
  // Carregar os dados e desenhar a tela são falhas diferentes: juntá-las num
  // catch só fazia erro de render aparecer como "confira se o site está em
  // HTTP", escondendo a causa real.
  let catalogo;
  try {
    catalogo = await carregarCatalogo();
  } catch (erro) {
    console.error('Falha ao carregar o catálogo da ficha:', erro);
  ...
  try {
    ...
    console.error('Falha ao desenhar a ficha:', erro);
    content.innerHTML = `<p class="ficha-erro-carregamento">Não foi possível desenhar esta ficha: ${erro.message}. Recarregue a página; se continuar, avise para corrigirmos.</p>`;
  }
}
