...
import { abrirPortal, inicializarPlataforma } from '../plataforma/portal.js?v=8';
...
function initCards() {
  document.querySelectorAll('.menu-card:not([data-wip])').forEach(card => {
    const destino = card.getAttribute('href') || (card.dataset.module ? `/${card.dataset.module}` : null);
    if (!destino) return;
...
// Busca global: itens/mundo liberados + personagens do jogador, com link pro módulo.
const _BUSCA_MODULOS = {
  loja: { rotulo: 'Itens', href: () => '/loja' },
  mundo: { rotulo: 'Mundo', href: () => '/mundo' },
  ficha: { rotulo: 'Ficha', href: ref => `/ficha#/personagem/${ref}` },
};

function initBusca(campanhaId) {
  const input = document.getElementById('menu-busca-input');
  const painel = document.getElementById('menu-busca-resultados');
  if (!input || !painel || !campanhaId) return;

  let timer = null;
  const fechar = () => { painel.hidden = true; painel.replaceChildren(); };
  ...
  input.addEventListener('input', () => {
    ...
    timer = setTimeout(async () => {
      try {
        const { conteudoApi } = await import('../plataforma/conteudoApi.js');
        const { resultados } = await conteudoApi.buscar(campanhaId, termo);
        painel.replaceChildren();
        if (!resultados.length) {
          const vazio = document.createElement('p');
          vazio.className = 'menu-busca-vazio';
          vazio.textContent = 'Nada encontrado na campanha.';
          painel.append(vazio);
        } else {
          resultados.forEach(resultado => {
            const modulo = _BUSCA_MODULOS[resultado.modulo] || { rotulo: resultado.modulo, href: () => '/' };
            const link = document.createElement('a');
            link.className = 'menu-busca-item';
            link.href = modulo.href(resultado.ref);
            const tag = document.createElement('span');
            tag.className = 'menu-busca-tag';
            tag.textContent = modulo.rotulo;
            const titulo = document.createElement('span');
            titulo.className = 'menu-busca-titulo';
            titulo.textContent = resultado.titulo;
            link.append(tag, titulo);
            painel.append(link);
          });
        }
        painel.hidden = false;
  ...
  document.addEventListener('click', evento => {
    if (!input.parentElement.contains(evento.target)) fechar();
  });
}
...
document.addEventListener('DOMContentLoaded', async () => {
  // Navegação primeiro: os cartões já funcionam enquanto conta e campanha são
  // revalidadas. O fundo animado entra quando o navegador estiver ocioso.
  initCards();
  const iniciarFundo = () => initCosmicTree();
  if ('requestIdleCallback' in window) requestIdleCallback(iniciarFundo, { timeout: 600 });
  else requestAnimationFrame(iniciarFundo);
  initCustomIcons();
  initSettings();
  initAtalhosPortal();
  ...
  initBusca(contexto?.campanha?.id);
});
