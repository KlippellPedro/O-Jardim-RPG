...
import { sessaoApi } from '../plataforma/sessaoApi.js?v=3';
import { criarLog } from './views/logView.js';
import { renderMesa } from './views/mesaView.js?v=3';
...
function desenhar() {
  ...
  renderMesa(content, estadoAtual, {
    ...
    sincronizarIniciativa: () => executar(
      () => sessaoApi.sincronizarIniciativa(estadoAtual.sessao.id),
      'Iniciativas fixas atualizadas e fila ordenada.',
    ),
    // Lista de monstros do bestiário para o mestre montar a cena (só busca).
    bestiario: () => sessaoApi.bestiario(campanhaId),
  });
