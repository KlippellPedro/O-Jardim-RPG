...
// Condição pode vir como string (formato antigo) ou {nome, turnos}. Estes
// helpers deixam a tela lidar com os dois sem se preocupar com o formato.
const nomeCondicao = cond => (typeof cond === 'string' ? cond : cond?.nome) || '';
const turnosCondicao = cond => (cond && typeof cond === 'object' ? cond.turnos : null) || null;
...
function cartaoParticipante(participante, estado, ctx) {
  ...
  if (participante.condicoes?.length) {
    ...
    participante.condicoes.forEach(condicao => {
      const nome = nomeCondicao(condicao);
      const turnos = turnosCondicao(condicao);
      const marca = elemento('span', 'sessao-condicao', turnos ? `${nome} · ${turnos}t` : nome);
      if (turnos) marca.dataset.temporaria = 'true';
      if (ctx.comando) {
        ...
        remover.addEventListener('click', () => ctx.alterar(participante.id, {
          condicoes: participante.condicoes.filter(item => nomeCondicao(item) !== nome),
        }));
...
function controlesDoMestre(participante, ctx) {
  ...
  CONDICOES_COMUNS
    .filter(item => !(participante.condicoes || []).some(c => nomeCondicao(c) === item))
    .forEach(item => condicao.append(new Option(item, item)));
  const turnos = document.createElement('input');
  turnos.type = 'number';
  turnos.min = '1';
  turnos.max = '999';
  turnos.placeholder = '∞';
  turnos.className = 'sessao-numero sessao-numero--turnos';
  turnos.title = 'Duração em turnos (vazio = permanente, até remover na mão)';
  turnos.setAttribute('aria-label', `Duração em turnos da condição para ${participante.nome}`);
  condicao.addEventListener('change', () => {
    if (!condicao.value) return;
    const t = Math.trunc(Number(turnos.value) || 0);
    ctx.alterar(participante.id, {
      condicoes: [...(participante.condicoes || []), { nome: condicao.value, turnos: t > 0 ? t : null }],
    });
    condicao.value = '';
    turnos.value = '';
  });
  controles.append(condicao, turnos);
...
function comandoDaMesa(estado, ctx) {
  ...
  // Personagens usam o número fixo da ficha; NPCs preservam o valor digitado.
  const atualizarIniciativas = botao(
    '↕ Atualizar iniciativas',
    'sessao-botao--secundario',
    () => ctx.sincronizarIniciativa(),
  );
  atualizarIniciativas.title = 'Sincroniza a iniciativa fixa das fichas e ordena do maior para o menor.';
  if (estado.sessao.em_combate) {
    acoes.append(
...
function formularioParticipante(ctx) {
  ...
  ctx.bestiario?.().then(({ monstros }) => {
    (monstros || []).forEach(m => {
      ...
      opcao.dataset.titulo = m.titulo;
      opcao.dataset.nivel = m.nivel ?? '';
      bestiario.append(opcao);
    });
    bestiario.disabled = false;
  }).catch(() => {});
  bestiario.addEventListener('change', () => {
    const opcao = bestiario.selectedOptions[0];
    if (!opcao || !opcao.value) return;
    nome.value = opcao.dataset.titulo || '';
    const nivel = Number(opcao.dataset.nivel) || 0;
    if (nivel > 0) vida.value = String(Math.max(10, nivel * 5));
    tipo.value = 'inimigo';
    nome.focus();
  });
  ...
  form.append(
    campoRotulado('Do bestiário', bestiario),
