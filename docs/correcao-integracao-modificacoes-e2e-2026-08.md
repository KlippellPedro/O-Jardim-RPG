...
**A 1 falha da plataforma naquela execução** (`test_create_character_bloqueia_raca_fora_da_arvore`) era pré-existente e não relacionada à etapa: o comportamento já permitia a criação fora da Árvore e notificava o Mestre, enquanto o teste antigo ainda esperava HTTP 422. A decisão final de produto de 13 de agosto de 2026 oficializou o comportamento permissivo: a Árvore orienta as regras, mas não bloqueia a liberdade do jogador na ficha. O teste foi substituído por `test_create_character_permite_raca_fora_da_arvore_e_notifica_mestre`, que comprova em Postgres real a criação, a persistência do personagem e o aviso ao Mestre.
...
- **Criação fora da Árvore** (seção 7): resolvida pela decisão final de produto de 13 de agosto de 2026. A criação permanece permitida e gera aviso ao Mestre; não há bloqueio HTTP 422 por divergência das regras da Árvore.
...
A cadeia de modificação (`dados reais → aplicacao/compatibilidade → pré-requisito → slots → exclusividade → compra/instalação`) está provada ponta a ponta contra banco real, incluindo o cenário combinado das duas camadas de validação. A cadeia de Mana (`uso → desconto → banco → sessão → SSE → HUD`) está provada nos 9 sub-casos pedidos, incluindo a verificação explícita da versão do evento `participante_atualizado` e um teste de concorrência real com lock de linha. As duas pendências técnicas que fecharam a etapa anterior estão resolvidas. A decisão de criação fora da Árvore também foi encerrada em 13 de agosto de 2026 com o contrato "permite e avisa"; permanece apenas a decisão separada sobre o alvo "veículo" isento.
